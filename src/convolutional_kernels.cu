...
{
    printf("forward_convolutional_layer_gpu, l.n = %d, l.groups = %d, l.size = %d, l.c = %d, l.out_w = %d, l.out_h = %d, l.xnor = %d, l.batch = %d, l.w = %d, l.h = %d, l.stride = %d, l.pad = %d, l.batch_normalize = %d, l.binary = %d, l.nweights = %d, l.activation = %d\n", l.n, l.groups, l.size, l.c, l.out_w, l.out_h, l.xnor, l.batch, l.w, l.h, l.stride, l.pad, l.batch_normalize, l.binary, l.nweights, l.activation);
    ...
    for(i = 0; i < l.batch; ++i){
        for(j = 0; j < l.groups; ++j){
...

            {
                ...
                sprintf(buffer, "yolo_gpu_weights_b_%d_g_%d_%dx%dx%dx%d.bin", i, j, l.size, l.size, l.c, l.n);
                fp = fopen(buffer, "wb");
                if(NULL == fp)
                {
                    printf("yolo_gpu_weights open error\n");
                }
                fwrite(a, l.nweights_gpu, sizeof(float), fp);
                fclose(fp);
            }
